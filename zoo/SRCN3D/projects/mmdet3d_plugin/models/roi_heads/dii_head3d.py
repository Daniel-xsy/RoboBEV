...
class DIIHead3D(BBoxHead):
    ...
    def __init__(self,
        ...
        #over load the self.fc_cls in BBoxHead
        self.fc_reg = nn.Linear(in_channels, self.code_size)

        assert self.reg_class_agnostic, 'DIIHead3D only ' \
            'suppport `reg_class_agnostic=True` '
        assert self.reg_decoded_bbox, 'DIIHead3D only ' \
            'suppport `reg_decoded_bbox=True`'

    def init_weights(self):
        """Use xavier initialization for all weight parameter and set
        classification head bias as a specific value when use focal loss."""
        super(DIIHead3D, self).init_weights()
        ...
        nn.init.constant_(self.fc_reg.bias.data[2:], 0.0)
    ...
    def forward(self, roi_feat, proposal_feat):
        ...
        Args:
            ...
            proposal_feat (Tensor): Intermediate feature get from
                DIIHead3D in last stage, has shape
                (batch_size, num_proposals, feature_dimensions)
        ...
        bbox_delta = self.fc_reg(reg_feat).view(N, num_proposals, self.code_size)
    ...
    #          **kwargs):
    #     """"Loss function of DIIHead3D, get loss of all images.
