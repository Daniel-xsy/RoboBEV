...
def custom_create_nuscenes_infos(root_path,
    ...
    d1_train_scenes = list(
        filter(lambda x: x in available_scene_names, d1_train_scenes))
    d1_val_scenes = list(filter(lambda x: x in available_scene_names, d1_val_scenes))
    d2_train_scenes = list(
        filter(lambda x: x in available_scene_names, d2_train_scenes))
    d2_val_scenes = list(filter(lambda x: x in available_scene_names, d2_val_scenes))

    d1_train_scenes = set([
        available_scenes[available_scene_names.index(s)]['token']
        for s in d1_train_scenes
    ])
    d1_val_scenes = set([
        available_scenes[available_scene_names.index(s)]['token']
        for s in d1_val_scenes
    ])
    d2_train_scenes = set([
        available_scenes[available_scene_names.index(s)]['token']
        for s in d2_train_scenes
    ])
    d2_val_scenes = set([
        available_scenes[available_scene_names.index(s)]['token']
    ...
    if domain_version == 'city2city':
        ...
        metadata['domain'] = 'city2city-sing'
